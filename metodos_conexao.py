...
def consultarBancoDados(connection, sql, dados):
    try:
        cursor = connection.cursor(prepared=True)
        cursor.execute(sql, dados)
        # O fetchone() pega a primeira linha de resultado do SELECT
        resultado = cursor.fetchone()
        return resultado[0] # Retorna o número que o COUNT(*) encontrou
    except mysql.connector.Error as err:
